...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // CORRECTION : process.env n'est pas disponible dans vite.config.js
      // au runtime Node — on utilise directement la variable d'environnement
      // via process.env (côté Node, pas import.meta.env qui est côté browser).
      // La valeur est injectée par docker-compose : VITE_API_PROXY_TARGET=http://api:5000
      // En dev local sans Docker, le fallback "http://localhost:5000" s'applique.
      "/api": {
