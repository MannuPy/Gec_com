...
## Documentation exhaustive — État au 19 juin 2026
...
| Module | Mode ML (scikit-learn présent) | Mode repli |
|---|---|---|
| ABC/XYZ | Règles pandas (déterministe) | Identique |
| Prévision demande | sklearn.LinearRegression + Prophet/XGBoost si dispo | Seasonal Naive |
| Scoring crédit | Random Forest + Logistic Regression cross-validés | Score pondéré par règles |
| Segmentation RFM | K-Means (4 clusters) | Segmentation par quartiles |
| Anomalies | Isolation Forest | Z-score (règle statistique) |

---

## 3. Module 1 — Classification ABC/XYZ des produits
...
**Algorithme utilisé :** `PANDAS_RULES_ABC_XYZ` (déterministe, pas de ML supervisé)
...
**Source SIMULATED (repli) :** en l'absence de données réelles (feature store vide), le module calcule `taux_retard` et `delai_moyen_remboursement_jours` de façon **déterministe** via un hash SHA-256 du `customer.id` alimentant un générateur aléatoire à graine fixe. Garanties :
...
**Source SIMULATED (déterministe) :** si historique insuffisant, le taux est calculé via `SHA-256(customer_id)` → graine aléatoire → `taux_retard ~ Beta(2, 5)`. Le résultat est **reproductible** (même input → même output) et **réaliste** (distribution Beta centrée sur ~15% de retards). La source est tracée dans `data_source = 'SIMULATED'`.
...
| SHA-256 pour données simulées | Random pur | Reproductibilité déterministe |
...
### 20.1 Limites de la version actuelle

**Données :**
- Scoring crédit basé partiellement sur des données simulées (pas de vrai historique de paiement en production initiale)
- Prévision de demande : Prophet/XGBoost optionnels → LinearRegression ou Seasonal Naive en production PythonAnywhere
- Pas de gestion des saisonnalités locales (Tabaski, saison des pluies) dans la version déployée — Prophet uniquement
