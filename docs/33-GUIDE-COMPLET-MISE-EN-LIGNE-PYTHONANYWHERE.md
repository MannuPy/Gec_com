...
**i) DISABLE_SSE** — désactiver les Server-Sent Events (obligatoire sur PythonAnywhere)

```
DISABLE_SSE=true
```

> PythonAnywhere ne supporte pas les connexions longue durée (SSE). Cette variable désactive les endpoints SSE côté Flask ; le frontend utilisera le polling.

**j) SENTRY_DSN** — monitoring d'erreurs (optionnel)

```
# Laisser vide ou commenter pour désactiver
SENTRY_DSN=
# Exemple si vous avez un projet Sentry : SENTRY_DSN=https://xxxx@sentry.io/yyyy
```
...
```json
{
  "status": "ok",
  "db": "connected",
  "ml_models": ["prophet", "random_forest", "isolation_forest", "kmeans", "apriori"]
}
```

> Les champs `db` et `ml_models` indiquent respectivement l'état de la connexion MySQL et les modèles ML chargés en mémoire. Si `ml_models` est vide, relancer le cron manuellement (voir §17.3).

Si vous voyez `{"status": "ok"}` (ou la réponse complète ci-dessus) → **l'API Flask fonctionne !** ✅
...
Utiliser les identifiants définis à l'étape 8.3 (f) pour l'admin:
- **Email :** `admin@gescom-bf.bf`
- **Mot de passe :** `admin@gescom-bf.bf`

Compte vendeur :
- **Email :** `vendeur@gescom-bf.bf`
- **Mot de passe :** ``Vendeur#2026``
...
*(Remplacer `mannu` par votre username dans les commandes suivantes)*

### 17.3 Ajouter la tâche planifiée ML (entraînement nocturne)

Le script `scripts/cron_train_all.py` (à la **racine du dépôt**, pas dans `backend/`) entraîne tous les modèles ML.

Dans l'onglet **Tasks** → **"Add a new scheduled task"** → remplir :

| Heure (UTC+0 ≈ heure BF) | Fréquence | Commande |
|---|---|---|
| 02:00 | Daily | `/home/mannu/.virtualenvs/gescom-bf/bin/python /home/mannu/gescom-bf/scripts/cron_train_all.py >> /home/mannu/gescom-bf/logs/cron_train.log 2>&1` |

> **Important :** la commande utilise le Python du virtualenv **complet** (`~/.virtualenvs/gescom-bf/bin/python`) afin que toutes les dépendances ML (scikit-learn, Prophet, mlxtend, shap…) soient disponibles. Si vous avez créé le virtualenv dans `backend/.venv` (étape 6), adaptez le chemin en conséquence :
> ```
> /home/mannu/gescom-bf/backend/.venv/bin/python /home/mannu/gescom-bf/scripts/cron_train_all.py >> /home/mannu/gescom-bf/logs/cron_train.log 2>&1
> ```

Cliquer **"Create"** pour valider la tâche.
