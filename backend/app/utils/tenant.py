"""
Utilitaires de résolution et d'application du schéma PostgreSQL d'un tenant.

Cf. docs/27-MODELE-SAAS-MULTITENANT.md (§27.2 - schema-per-tenant, §27.7 -
résolution du tenant, table d'index `public.user_index`).
"""
import re
import unicodedata
...
# "public" (tenant par défaut / V1 mono-tenant) ou "tenant_<slug>".
...
def slugify_company_name(name: str) -> str:
    ...
    schéma PostgreSQL (RF-01) : minuscules, chiffres et underscores uniquement.
    """
...
def schema_name_for_company(name: str) -> str:
    """Génère le nom de schéma PostgreSQL dédié d'une nouvelle entreprise."""
...
def set_search_path(schema_name: str) -> None:
    ...
    Le schéma `public` reste toujours accessible en second afin que les
    tables du registre central (`companies`, `user_index`) restent
    consultables quel que soit le tenant courant.

    Lève `INVALID_TENANT_CONTEXT` (403) si `schema_name` ne respecte pas le
    format attendu (`public` ou `tenant_<slug>`), ce qui empêche toute
    injection SQL via cette valeur (issue d'un claim JWT) — cette validation
    est effectuée quel que soit le moteur de base de données.

    Sur MySQL (déploiement mono-tenant, ex. PythonAnywhere : une seule base
    de données, pas de notion de schéma équivalente à PostgreSQL), la
    commande `SET search_path` n'existe pas et n'est donc pas exécutée :
    ...
    if schema_name == "public":
        ...
        # `schema_name` est garanti conforme à `_SCHEMA_NAME_RE` ci-dessus
        # (caractères [a-z0-9_] uniquement) : l'interpolation est sûre.
        db.session.execute(text(f'SET search_path TO "{schema_name}", public'))


def get_schema_for_email(email: str) -> str:
    """Retrouve le schéma tenant associé à un email via `public.user_index`
    (§27.7). Retombe sur le tenant par défaut (`DEFAULT_TENANT_SCHEMA`,
    schéma `public` en V1 mono-tenant) si l'email n'est pas encore indexé -
    ce qui couvre les comptes de démonstration créés avant l'introduction
    du multi-tenant.
    """
...
def register_user_index(email: str, schema_name: str) -> None:
    """Crée ou met à jour l'entrée `public.user_index` pour cet email."""
    email = email.lower()
